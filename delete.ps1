...
try {
    ...
    $currentAccount = Get-ArdaOnlineAccount -ExternalUserId $($actionContext.References.Account) -Headers $headers

    if ($actionContext.AccountCorrelated -eq $true -and $null -ne $currentAccount) {
        $action = 'DeleteAccount'
        $dryRunMessage = "Delete ArdaOnline account: [$($actionContext.References.Account)] for person: [$($personContext.Person.DisplayName)] will be executed during enforcement"
    } else {
        $action = 'NotFound'
    ...
    if (-not($actionContext.DryRun -eq $true)) {
        switch ($action) {
            'DeleteAccount' {
                Write-Verbose "Delete ArdaOnline account with accountReference: [$($actionContext.References.Account)]"

                # Add 'externalUserId' and 'email' because both are mandatory
                $null = Set-ArdaOnlineAccount -Account $($actionContext.Data) -Headers $headers
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not delete ArdaOnline account. Error: $($errorObj.FriendlyMessage)"
        Write-Verbose "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not delete ArdaOnline account. Error: $($ex.Exception.Message)"
